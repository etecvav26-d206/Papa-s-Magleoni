...
try {
    ...
    foreach ($statements as $statement) {
        if (preg_match('/^CREATE TABLE /i', $statement)) { $pdo->exec($statement); }
    }
    ...
    $foreignKeys = $pdo->query("SELECT CONSTRAINT_NAME FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='pizzas' AND COLUMN_NAME='categoria_id' AND REFERENCED_TABLE_NAME='categorias'")->fetchAll();
    if (!$foreignKeys) {
        $pdo->exec('ALTER TABLE pizzas ADD CONSTRAINT fk_pizzas_categoria FOREIGN KEY (categoria_id) REFERENCES categorias(id) ON DELETE SET NULL');
    }
    $pdo->beginTransaction();
    foreach ($statements as $statement) {
        if (preg_match('/^INSERT INTO /i', $statement)) { $pdo->exec($statement); }
    }
    $pdo->commit();
